...
console.log(func) //here it will not delete the a, it will simply return 5
//as a is the local variable of ithe nner function
//delete will be used to delete properties of objs, not local variables

//Dynamic Key value to Object
const property = 'firstname'
const name = 'test';
const user = {
    property: name, //gives property test
...
//if we have 2 keys with he same name, if will replace the value with the new value for the first key found
...
//a[b] -> b can not be assigned as a key to a object as it's a object itself, it could have been assigned if b were a string -> "[obj, obj]"=123
...
//usecase -> storing data into local storage: setItems-> as we cannot store obj directly to local storage
...
//QUE 5
console.log([..."abcde"]) 
//output-> it will spread all the characters into array of Characters ['a','b']
...
console.log(admin);
//output: it will add all the properties of user to admin object

//QUE 7
//what is destructing obj: taking out specific properties of objects
const testDestruct = {
    nameProperty:'test',
    fullname:{
        firstname:'vaish',
...
//we simply share the reference not the properties of the object 
//as we change the value of x.greeting and y has a reference of X y's greeting will also change

//QUE 10
console.log({a:1}=={a:1}); //false
console.log({b:1}== {b:1});// even if it === then also false
// as each object has its own memory space
...
//shallow:  a new obj that we created shared the same reference of those source obj
// results in f you change either of them changes will  be occur to both
//DEEP: source and copy obj are completely independent | no reference
